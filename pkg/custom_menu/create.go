...
const (
	...
	TypeView            Type = "view"
	TypeScancodePush    Type = "scancode_push"
	TypeScancodeWaitMsg Type = "scancode_waitmsg"
	TypePicSysPhoto     Type = "pic_sysphoto"
	TypePicPhotoOrAlbum Type = "pic_photo_or_album"
	TypePicWeixin       Type = "pic_weixin"
	TypeLocationSelect  Type = "location_select"
	TypeMediaId         Type = "media_id"
	TypeViewLimited     Type = "view_limited"
	TypeMiniProgram     Type = "miniprogram"
)
...
type Button struct {
	...
	Type      Type     `json:"type,omitempty"`       // 菜单的响应动作类型，view表示网页类型，click表示点击类型，miniprogram表示小程序类型
	SubButton []Button `json:"sub_button,omitempty"` // 子菜单, 1-5 个
	Url       string   `json:"url,omitempty"`        // view, miniprogram 类型必须, 网页 链接，用户点击菜单可打开链接，不超过1024字节。 type为miniprogram时，不支持小程序的老版本客户端将打开本url。
	Key       string   `json:"key,omitempty"`        // click, pic_sysphoto, pic_photo_or_album, pic_weixin, location_select 类型必须, 菜单KEY值，用于消息接口推送，不超过128字节
	Appid     string   `json:"appid,omitempty"`      // miniprogram 类型必须, 小程序的appid（仅认证公众号可配置）
	Pagepath  string   `json:"pagepath,omitempty"`   // miniprogram 类型必须, 小程序的页面路径
	MediaId   string   `json:"media_id,omitempty"`   // media_id, view_limited 类型必须, 调用新增永久素材接口返回的合法media_id
}
